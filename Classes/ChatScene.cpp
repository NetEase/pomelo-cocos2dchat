...
#define LongSentencesExample "fantasyni:\n Lorem ipsum dolor sit amet, consectetur adipisicing elit, saliqua."
#define FONT_NAME           "Thonburi"
#define FONT_SIZE           36
#define m_nCharLimit		20
...
static CCLayer* m_pItemLayer = NULL;
static int cur = 0;
...
CCLayer* Chat::sharedMItemLayer()
{
    if(m_pItemLayer == NULL) {
        m_pItemLayer = new CCLayer();
        m_pItemLayer->init();
    }
    return m_pItemLayer;
}
...
bool Chat::init()
{
    ...
    for (int i = 0; i < TESTS_COUNT; ++i) {
        CCLabelTTF* label = CCLabelTTF::create(global_text[i].c_str(), "Arial", 20,CCSizeMake(480,50),kCCTextAlignmentLeft);
...
void Chat::onEnter()
{
    ...
    //schedule(schedule_selector(Chat::updateUICallback),0,false,0);
...
void Chat::updateUICallback(float delta)
{
    ...
    for(unsigned int i = cur; i < messageQueue->count(); i++, cur++) {
        ...
        CCLabelTTF* label = CCLabelTTF::create(content.c_str(), "Arial", 20,CCSizeMake(VisibleRect::getVisibleRect().size.width,50),kCCTextAlignmentLeft);
        label->setColor(ccc3(32,32,32));
        label->setAnchorPoint(ccp(0,0));
...
void Chat::ccTouchesMoved(CCSet *pTouches, CCEvent *pEvent)
{
    ...
    unsigned int count = m_layer->getChildrenCount();
    //CCLog("%d %d", m_pItemLayer->getChildrenCount(),m_tBeginPos.y);
    CCSetIterator it = pTouches->begin();
    CCTouch* touch = (CCTouch*)(*it);
    ...
    //CCLOG("nextPos: %f, %f",nextPos.x,nextPos.y);
    if (nextPos.y < 0.0f) {
        m_layer->setPosition(ccp(0,0));
        //m_pItemMenu->setPosition(VisibleRect::left().x, VisibleRect::top().y - 10);
        return;
    }
